...
/// 静的メンバ変数の実体
/// </summary>
const std::string OBJHighModel::baseDirectory = "Resources/";
ID3D12Device* OBJHighModel::device = nullptr;
UINT OBJHighModel::descriptorHandleIncrementSize = 0;

void OBJHighModel::SetDevice(ID3D12Device* device)
{
	// 再初期化チェック
	assert(!OBJHighModel::device);

	OBJHighModel::device = device;

	// メッシュの静的初期化
	Mesh::SetDevice(device);
}

OBJHighModel* OBJHighModel::CreateFromOBJ(const std::string& modelname, bool smoothing)
{
	// メモリ確保
	OBJHighModel* instance = new OBJHighModel;
	instance->Initialize(modelname, smoothing);

	return instance;
}

OBJHighModel::~OBJHighModel()
{
	for (auto m : meshes) {
		delete m;
	}
...
void OBJHighModel::Initialize(const std::string& modelname, bool smoothing)
{
	// モデル読み込み
...
void OBJHighModel::LoadModel(const std::string& modelname, bool smoothing)
{
...
void OBJHighModel::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void OBJHighModel::AddMaterial(Material* material)
{
	// コンテナに登録
	materials.emplace(material->name, material);
}

void OBJHighModel::CreateDescriptorHeap()
{
...
void OBJHighModel::LoadTextures()
{
	int textureIndex = 0;
...
void OBJHighModel::Draw(ID3D12GraphicsCommandList* cmdList)
{
	// デスクリプタヒープの配列
	if (descHeap) {
