...
/// 静的メンバ変数の実体
/// </summary>
const std::string ObjFileModel::baseDirectory = "Resources/";
ID3D12Device* ObjFileModel::device = nullptr;
UINT ObjFileModel::descriptorHandleIncrementSize = 0;

void ObjFileModel::SetDevice(ID3D12Device* device)
{
	// 再初期化チェック
	assert(!ObjFileModel::device);

	ObjFileModel::device = device;

	// メッシュの静的初期化
	Mesh::SetDevice(device);
}

ObjFileModel* ObjFileModel::CreateFromOBJ(const std::string& modelname, bool smoothing)
{
	// メモリ確保
	ObjFileModel* instance = new ObjFileModel;
...
void ObjFileModel::Initialize(const std::string& modelname, bool smoothing)
{
	// モデル読み込み
...
void ObjFileModel::LoadModel(const std::string& modelname, bool smoothing)
{
...
void ObjFileModel::LoadMaterial(const std::string& directoryPath, const std::string& filename)
{
	// ファイルストリーム
	std::ifstream file;
	// マテリアルファイルを開く
...
void ObjFileModel::AddMaterial(Material* material)
{
	// コンテナに登録
	materials.emplace(material->name, material);
}

void ObjFileModel::CreateDescriptorHeap()
{
...
void ObjFileModel::LoadTextures()
{
	int textureIndex = 0;
...
void ObjFileModel::Draw(ID3D12GraphicsCommandList* cmdList)
{
	// デスクリプタヒープの配列
	if (descHeap) {
